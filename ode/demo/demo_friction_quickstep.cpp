...
#define FORCE 50.0	// force applied to box[i][j] = (j+1) * FORCE
...
#define ITERS 1000      // number of different masses to try
...
int main (int argc, char **argv)
{
  ...
  for (i=0; i<N1; i++) {
    for (j=0; j<N2; j++) {
      ...
      dBodySetPosition (body[i][j],i*2*LENGTH,j*2*LENGTH,GHEIGHT+HEIGHT*0.5);
